...
public class RequireAzureTestCaseAttribute
{
    ...
    [Fact]
    public void OneShotTrait_MustBeOnMethodsNotClasses()
    {
        var violators = _testClasses
            .Where(t => t.GetCustomAttributesData()
                .Any(a => a.AttributeType.Name == "TraitAttribute"
                    && a.ConstructorArguments.Count == 2
                    && a.ConstructorArguments[0].Value?.ToString() == "Category"
                    && a.ConstructorArguments[1].Value?.ToString() == "OneShot"))
            .Select(t => t.Name)
            .ToList();

        Assert.True(violators.Count == 0,
            $"The following test classes have [Trait(\"Category\", \"OneShot\")] at class level (must be on methods):\n  - {string.Join("\n  - ", violators)}");
    }
